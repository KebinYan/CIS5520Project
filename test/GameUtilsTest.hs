...
testGenerateRandomCandyList = TestCase $ do
    ...
    let candyShapes = map (shapeName . candyDef) candies
    assertEqual "Correct number of candies generated" 
        10 (length generatedCandies)
...
testComputeArbitraryPositions = TestCase $ do
    let boardSize = (3, 3)
        positions = computeArbitraryPositions boardSize (2, 1) 
                    [(Coordinate 0, All), (All, Coordinate 0)]
        expected = sort [
...
testGenerateSpecialEffect2 = TestCase $ do
    ...
    let positionsCleared = computeArbitraryPositions (3, 3) (2, 1) 
                        [(Coordinate 0, All), (All, Coordinate 0)]
        candiesAtClearedPositions = map (getCandyAt newBoard) positionsCleared
...
testGetCandyAt = TestCase $ do
    assertEqual "Get valid candy" (Just candy2) 
        (getCandyAt (board initialGrid) (Coordinate 0, Coordinate 1))
    assertEqual "Get invalid candy" Nothing 
        (getCandyAt (board initialGrid) (Coordinate 3, Coordinate 3))

testSetCandyAt :: Test
testSetCandyAt = TestCase $ do
    let updatedBoard = 
            setCandyAt (board initialGrid) (Coordinate 0, Coordinate 1) candy6
...
testSetCandyAtBoundary = TestCase $ do
    let updatedBoard = 
            setCandyAt (board initialGrid) (Coordinate (-1), Coordinate 0) candy6
    assertEqual "Set candy at negative index should not change board" 
        (board initialGrid) updatedBoard
    let updatedBoard2 = 
            setCandyAt (board initialGrid) (Coordinate 0, Coordinate 100) candy6
    assertEqual "Set candy at out-of-bounds index should not change board" 
        (board initialGrid) updatedBoard2
...
testGetCandyAtBoundary = TestCase $ do
    ...
    assertEqual "Get candy at negative index should return Nothing" 
        Nothing result
    let result2 = getCandyAt (board initialGrid) (Coordinate 0, Coordinate 100)
    assertEqual "Get candy at out-of-bounds index should return Nothing" 
        Nothing result2
...
testValidCoordinate = TestCase $ do
    assertBool "Valid coordinate" 
        (validCoordinate (board initialGrid) (Coordinate 0, Coordinate 0))
    assertBool "Invalid coordinate" 
        (not $ validCoordinate (board initialGrid) (Coordinate 3, Coordinate 0))
    assertBool "Invalid coordinate" 
        (not $ validCoordinate (board initialGrid) (Coordinate 0, Coordinate (-1)))
    assertBool "Invalid coordinate" 
        (not $ validCoordinate (board initialGrid) (Coordinate 0, All))
...
testRedeemSpecialCandy = TestList [
    "Create a striped row candy with 4 candies" ~:
        do result <- redeemSpecialCandy 4 (specialCandies crushableGrid)
           assertEqual "Redeem special candy with 4 candies" 
                (Just candy7) result,
    ...
    "Create a striped cross candy with 6 candies" ~:
        do result <- redeemSpecialCandy 6 (specialCandies crushableGrid)
           assertEqual "Redeem special candy with 6 candies" 
                (Just candy6) result,
    "No special candy with fewer than 4 candies" ~:
        do result <- redeemSpecialCandy 3 (specialCandies crushableGrid)
           assertEqual "No special candy with fewer than 4 candies" 
                Nothing result
  ]
...
testFillBoard = TestCase $ do
    newBoard <- 
        fillBoard (board gridWithEmptyCandy) (normalCandies gridWithEmptyCandy)
...
testFormatBlock :: Test
testFormatBlock = TestCase $ do
    let result = formatBlock 5 10 20
    let expected = "dimension: 5\nmaxSteps: 10\nscore: 20"
    assertEqual "formatBlock test" expected result

testSplitBlocks :: Test
testSplitBlocks = TestCase $ do
    let input = "block1\n\nblock2\n\nblock3"
    let result = splitBlocks input
    let expected = ["block1", "block2", "block3"]
    assertEqual "splitBlocks test" expected result

testFindBlock :: Test
testFindBlock = TestCase $ do
...
prop_validCoordinatesCleared :: GameConst -> Property
prop_validCoordinatesCleared d = monadicIO $ do
    ...
    boardAfterEffect <- run $ 
        return (generateSpecialEffect specialCandy coord board)
...
prop_noUnintendedModifications :: GameConst -> Property
...
prop_fillBoardIdempotency :: GameConst -> Property
...
-- Property: Applying the same special effect twice is equivalent to applying it 
-- once
prop_specialEffectIdempotency :: GameConst -> Property
prop_specialEffectIdempotency d = monadicIO $ do
    ...
    boardAfterFirst <- run $ 
        return (generateSpecialEffect specialCandy coord board)
    boardAfterSecond <- run $ 
        return (generateSpecialEffect specialCandy coord boardAfterFirst)
    Test.QuickCheck.Monadic.assert $ boardAfterFirst == boardAfterSecond

-- Property: Applying two special effects in any order is equivalent to 
-- applying them in the opposite order
prop_specialEffectAssociativity :: GameConst -> Property
...
-- Property: Applying generateSpecialEffect is equivalent to applying 
-- clearPosition if the candy at the coordinate is a special candy
prop_specialEffectEquivalent :: GameConst -> Property
...
-- Property: No EmptyCandy remains after applying generateSpecialEffect 
-- and fillBoard
prop_noEmptyCandyAfterFill :: GameConst -> Property
...
-- Property: Setting and then getting a candy at a valid coordinate retrieves 
-- the same candy
prop_setGetCandyAt :: GameConst -> Property
...
-- Property: Write and read consistency for game info
prop_writeReadConsistency :: GameConst -> Property
prop_writeReadConsistency gameConst = ioProperty $ do
    gameState <- generate (genGameState gameConst)
...
runUnitTests = runTestTT $ TestList [
    ...
    TestLabel "testClearPositionWithSpecialEffect" 
                testClearPositionWithSpecialEffect,
...
runQuickCheckTests = do
    ...
    putStrLn "prop_writeReadConsistency:"
    quickCheck prop_writeReadConsistency
