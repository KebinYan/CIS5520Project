...
unsafeEasy = unsafePerformIO $ parseFile "config/easy.json"

unsafeMedium :: Either ParseError Difficulty
unsafeMedium = unsafePerformIO $ parseFile "config/medium.json"

unsafeHard :: Either ParseError Difficulty
unsafeHard = unsafePerformIO $ parseFile "config/hard.json"

-- Define a helper function to filter successful parses
validDifficulties :: [Difficulty]
validDifficulties =
  case (unsafeEasy, unsafeMedium, unsafeHard) of
    (Right e, Right m, Right h) -> [e, m, h]
    _ -> error "Failed to parse one or more difficulty configurations"
...
instance Arbitrary Difficulty where
    ...
    arbitrary = elements validDifficulties
